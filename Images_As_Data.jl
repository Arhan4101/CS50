...
# ╔═╡ 65fe02a0-8ee6-11eb-2c4c-5f492127076d
new_phil=copy(philip)

# ╔═╡ 3a643f50-8ee7-11eb-2533-152021bb30dd
begin
 	new_phil[100:200,100:200].=RGB(1.0,0.0,0.0) 
	new_phil
end
...
# ╔═╡ 53324992-8eed-11eb-3784-0f6132bf4826
function dim(color::AbstractRGB)
	
	return RGB(color.r/3, color.g/3, color.b/3)
end
...
# ╔═╡ 275b14d0-8efe-11eb-12c0-c33ce99663d9
function redify(color::AbstractRGB)
	
	return RGB(color.r, 0.0, 0.0)
end

# ╔═╡ 3b25c320-8efe-11eb-0918-41efc7febe6c
function greenify(color::AbstractRGB)
	
	return RGB(0.0, color.g, 0.0)
end

# ╔═╡ f9c52e00-8eeb-11eb-31ef-d58e4eeb56d7
function blueify(color::AbstractRGB)
	
	return RGB(0.0, 0.0, color.b)
end

# ╔═╡ 4db2c14e-8efe-11eb-0f25-c540afb03f22
redify.(philip)

# ╔═╡ 73cc0540-8efe-11eb-0aab-1beeff997d63
greenify.(philip)

# ╔═╡ 123807a0-8eec-11eb-1c2a-6d734e3a491f
blueify.(philip)

# ╔═╡ 74d28880-8eed-11eb-3d97-eb36b92ad4f8
invert.(philip)

# ╔═╡ 7b39a1e0-8eed-11eb-0b83-cf1fd059dfb9
dim.(philip)
...
let
	temp = copy(philip_head)
	temp[50:300, 50:100] .= RGB(0.5, 0.0, 1.0)
	temp
end
...
[redify.(philip_head)                   reverse(invert.(philip_head), dims=2)
 reverse(blueify.(philip_head), dims=1)  rot180(greenify.(philip_head))]
